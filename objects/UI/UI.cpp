...
namespace cellworld{
    ...
    void UI::CreationOfTheWorld()
    {
        ...
        if (ImGui::Button("Reset", { width_ * 0.3f,height_ * 0.1f })) {
            size_x = 100;
            size_y = 50;
            seed_ = 0;
        ...
        if (ImGui::Button("Create new world", { width_ * 0.3f, height_ * 0.1f })) {
            ...
            init_field_=Field(size_x,size_y);
    ...
    {   
        ...
        for (int i=file_names.size()-1; i>=0 ; --i) {
            ...
            if (ImGui::Button("Load")) {
                loadWorld(file_names[i].c_str(), &init_field_, &Creature::coeff_, seed_);
                sceneUpdate(simulation_of_the_world);
            }
            ImGui::SameLine();
    ...
    void UI::SimulationOfTheWorld()
    {
        ...
        if (scene_is_changed_) {
            pause=1;
            visualise=1;
            texture=init_field_.createTexture();
            file_name="New world";
        }  
        ...
        if (ImGui::Button("Save as")) {
            if (file_names_.findFileName(file_name.c_str())) {
                ...
                saveWorld(new_name.c_str(), &init_field_, &Creature::coeff_, seed_);
                file_names_.addFileName(new_name.c_str());
                file_names_.saveFileNames();
            }
            else{
                saveWorld(file_name.c_str(), &init_field_, &Creature::coeff_, seed_);
        ...
        if (ImGui::Button("Make one step")) {
            init_field_.updatePositions();
            init_field_.updateStates();
        }

        ImGui::SameLine();
        if (ImGui::Button("Spawn Creature")) {
            Position pos = init_field_.generatePosition();
            init_field_.spawnCreature(pos);
        }
        ImGui::SameLine();
        if (ImGui::Button("Return")) {
            sceneUpdate(start_screen);
            init_field_.deleteTexture();
        }
        ImGui::SameLine();
        ...
        if (visualise) {
            ...
            init_field_.updateTexture();
            ImGui::Image(texture, { width_ * 0.98f,height_ * 0.85f });
            ImGui::End();
        }
    }
